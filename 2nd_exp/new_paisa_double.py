  ...
  for elem in found:
    if len(elem)>40:
      for verb in list_conj_vb:
        if verb in elem and elem not in sent:
          sent.append(elem[2:])
          if len(sent)%5000 == 0:
            print(elem[2:])
...
print(f"Extracting CnTp, CpTn, CpTp and CnTn types of sentences from PAISA...")
...
for s in sent:
    found = re.search(negC_patt, s)
    if found:
        double = re.search(negT_patt, s)
        if double:
            CnTn.append(s)
        if not double:
            CnTp.append(s)
    else:
        found2 = re.search(negT_patt, s)
        if found2:
            CpTn.append(s)
        if not found2:
            CpTp.append(s)
...
print(f"len CpTn = {CpTn[0]}")
print(f"len CnTp = {len(CnTp)}")
print(f"len CpTn = {CnTp[0]}")
print(f"len CnTn = {len(CnTn)}")
print(f"len CpTn = {CnTn[0]}")
print(f"len CpTp = {len(CpTp)}")
print(f"len CpTn = {CpTp[0]}")

dump(CpTp, "../Inputs/paisa_CpTp.joblib")
...
print(f"Extracting the CLS encodings from CpTn/CnTp/CpTp/CnTn sentences from PAISA...")

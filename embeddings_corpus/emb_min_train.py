...
ls = []
n = 0
for vb in list(verb_stats.head(700)["lemma"]):
    ls.append((vb, n))
    n+=1

for vb, n in ls:
    vb_perc_mn[vb] = list(verb_stats.head(700)["perc neg"])[n]
...
test_set, lab_set = create_test_set(vb_perc_mn, embs)
...
dump(scaler, f"scaler_min.joblib")
  ...
  for a in [1e-5, 1e-4, 1e-3]:
    for solv in ["adam", "sgd"]:
      ...
      clf = MLPClassifier(solver = solv, alpha = a,
                    hidden_layer_sizes=hl, random_state = 1, max_iter=200)
      ...
      dump(clf, f"classifiers/classifier_min_{n}.joblib")
    
with open(f"MIN_TRAIN.txt", "w") as res_file:
    res_file.write("MIN TRAIN\n")
with open(f"MIN_TRAIN.txt", "a") as res_file:
    for scores in emb_result:
        res_file.write(scores)

with open(f"MIN_TRAIN_LIST.txt", "w") as res_file:
    res_file.write("MIN TRAIN\n[")
with open(f"MIN_TRAIN_LIST.txt", "a") as res_file:

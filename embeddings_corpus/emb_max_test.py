...
with open(f"100MAX_ALLRANKS_TEST.txt", "w") as file:
    file.write("100 MAX ALL-RANKS TEST\n\n")

with open(f"100MAX_ALLRANKS_LIST.txt", "w") as file:
    file.write("100 MAX ALL-RANKS LISTS\n\n")


scaler = load(f"scaler_100max.joblib")
...
for rank in vbs_ranks.keys():
    ...
    n=0
    for n in range(1,13):
        clf = load(f"classifiers/classifier_100max_{n}.joblib")
    ...
    with open(f"100MAX_ALLRANKS_TEST.txt", "w") as res_file:
        res_file.write(f"{rank-99}--{rank} ranks\n")
    with open(f"100MAX_ALLRANKS_TEST.txt", "a") as res_file:
        for scores in emb_result:
            res_file.write(scores)
    
    with open(f"100MAX_ALLRANKS_LIST.txt", "a") as acc_file:
